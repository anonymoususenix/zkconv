...
impl<E: Pairing> StructuredReferenceString<E> for MultilinearUniversalParams<E> {
    ...
    fn gen_srs<R: Rng>(rng: &mut R, num_vars: usize) -> Self {
        ...
        // };
        let pp_g = {
            // Create the preprocessing struct for scalar multiplication
            let preprocessing = BatchMulPreprocessing::new(g, total_scalars);

            // Perform batch scalar multiplication, convert to G1, and normalize the results
            let g1_points: Vec<E::G1> = preprocessing
                .batch_mul(&pp_powers)
                .into_iter()
                .map(|g1_affine| g1_affine.into_group())
                .collect();
            E::G1::normalize_batch(&g1_points)
        };
        ...
        let h_mask = {
            // Create the preprocessing struct for scalar multiplication
            let preprocessing = BatchMulPreprocessing::new(h, num_vars);

            // Perform batch scalar multiplication, convert to G2, and normalize the results
            let g2_points: Vec<E::G2> = preprocessing
                .batch_mul(&t)
                .into_iter()
                .map(|g2_affine| g2_affine.into_group())
                .collect();
            E::G2::normalize_batch(&g2_points)
        };
