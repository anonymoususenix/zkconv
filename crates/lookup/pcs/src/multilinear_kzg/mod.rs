...
use ark_std::{marker::PhantomData, rand::Rng, One, Zero};
use data_structures::*;
use std::ops::Mul;
...
impl<E: Pairing> PolynomialCommitmentScheme<E> for MultilinearKzgPCS<E> {
    ...
    fn commit(prover_param: &Self::ProverParam, eval: &Vec<E::ScalarField>) -> Self::Commitment {
        ...
        let commitment =
            E::G1::msm_unchecked(&prover_param.powers_of_g[ignored].evals, eval).into_affine();
        commitment
    }
...
pub fn verify_internal<E: Pairing>(
    ...
    // };
    let h_mul: Vec<E::G2> = {
        // Create the preprocessing struct for the scalar multiplication
        let preprocessing = BatchMulPreprocessing::new(verifier_param.h.into_group(), num_vars);

        // Use the preprocessed data for batch scalar multiplication
        preprocessing
            .batch_mul(point)
            .into_iter()
            .map(|g2_affine| g2_affine.into_group())
            .collect()
    };

    let ignored = verifier_param.num_vars - num_vars;
    let h_vec: Vec<_> = (0..num_vars)
    ...
    let mut pairings: Vec<_> = proof
        ...
        .zip(h_vec.into_iter().map(E::G2Prepared::from))
        .collect();

    // add pairing e(g * v - c, g)
    pairings.push((
        E::G1Prepared::from((verifier_param.g.mul(value) - commitment.into_group()).into_affine()),
...
mod tests {
    ...
    fn test_single_commit() {
        ...
        let commit =
            <E as Pairing>::G1::msm_unchecked(&vec![commit_1, commit_2], &[F::from(1), F::from(1)])
                .into_affine();
