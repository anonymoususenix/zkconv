...
impl PermCheck {
    pub fn prove<F: PrimeField>(
        ...
        let (proof_0, challenges_0, value_0) = GrandProdCheck::prove(evals_0, transcript);
        proof.extend(proof_0);
        let (proof_1, challenges_1, value_1) = GrandProdCheck::prove(evals_1, transcript);
        proof.extend(proof_1);
        (
            proof,
            [challenges_0, challenges_1],
            [value_0 - r, value_1 - r],
        )
    }
    ...
    pub fn verify<F: PrimeField>(
        ...
        // ) -> bool {
        let r: F = get_and_append_challenge(transcript, b"Internal round");
        let (prod_0, challenge_0, value_0) = GrandProdCheck::verify(layer_num, transcript, proof);
        let (prod_1, challenge_1, value_1) = GrandProdCheck::verify(layer_num, transcript, proof);
